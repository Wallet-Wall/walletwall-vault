...
> none of which is an audit or formal verification, and each of which has its own
> scoped result under its own vocabulary:
>
> - the **stateful campaign** currently carries disclosed **SUSTAINED** defects,
>   listed with `knownGaps` and `whatThisDoesNotProve` in
>   `STATEFUL_AUTHORITY_EVIDENCE.json`;
> - the **authority-path checker** emits a per-entry **verdict** into
>   `AUTHORITY_CENSUS.json`;
> - the **scanner triage** assigns a per-finding **classification** in
>   `slither-triage.json`, receipted by `SCANNER_EVIDENCE.json`.
>
> `SUSTAINED` is the stateful ledger's disposition and belongs to it alone; the
> other two are not described in that vocabulary. Read each artifact before
> quoting any result here.
>
...
> identity**, kept because those reproducibility properties — zero
> `immutableReferences`, byte-identical rebuilds, clone/CREATE2 recovery — were
> established at that historical head. The BYTE COUNTS below are superseded.
>
> **Current measurements and reproducibility results are owned by
> `MEASUREMENTS.json` and `reproduce.ts`**, which CI regenerates and independently
> re-derives. This section makes no claim about any head other than `79e05a34`.
...
| selectors                                                    | 40 — **13 state-changing** (this table is the 14,339-byte head's identity, kept as history; the W2 kernel (Commit A `c182db10`) has 46 selectors, 14 state-changing — see `AUTHORITY.md` §4) |

## Static analysis coverage

| Scanner     | Status                                                                                                                                                                                                       |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **solhint** | **RUN in CI** (`vNext Kernel / Prototype Tests`) — 32 warnings, 0 errors at the head this row was written for; 34 at `e6964aeb`, **36 on the W2 kernel (Commit A)** (+2, both in the kernel: the new `RecoveryCancelledByQuorum` event's `gas-indexed-events` hint and the half-open `>= expiresAt` in `executeRecovery` flagged by `gas-strict-inequalities`), still 0 errors; **41 from lane SD-11** (+5, none in the kernel: four in the byte-identical copy of the production `ImmutableAttestationPQCVerifier` and one `immutable-vars-naming` on the factory's `verifierAuthority`)                                                                                                                                     |
| **Slither** | **RUN in CI** (`vNext Kernel / Slither`, path-scoped, branch-unrestricted) — every finding triaged firsthand in `slither-triage.json`; see `SCANNER_EVIDENCE.json` for the receipt. The scan emits SARIF **and** raw JSON from one execution, and the same job then fails on any untriaged finding, stale entry, ambiguous identity or scanner-input scope mismatch. Findings are keyed by SEMANTIC IDENTITY (no line numbers), so a finding that only moves keeps its adjudication; the v1 locator keys are retained per entry as history — `SCANNER_IDENTITY_CORRECTION_RECORD.md`, and `W2S_SCANNER_TRIAGE_RECORD.md` for the earlier re-key at `77ea92cf` |
