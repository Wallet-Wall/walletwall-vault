 ...
 * adversarially as the defect it closes. Lane SD-8 (2026-09-14) then re-derived
 * SD-8 ITSELF — the one entry that had only ever been written, never measured —
 * and found its judging party, its length scope, its edge scope and its key-length
 * figure all wrong while its verdict stood; it stays SUSTAINED, corrected in place.
 *
...
export const SUSTAINED_DEFECTS: readonly SustainedDefect[] = [
  ...
  },
  {
    ...
    // test/Sd8KeyWellFormednessAdjudication.test.ts) measured the same gap at FOUR sites and found
    // this entry's earlier wording wrong in four respects while its verdict stood. The entry is
    // CORRECTED RATHER THAN RE-IDENTIFIED, on the precedent of SD-1: SD-6 and SD-7 name this id as
    // their residual, every prior receipt carries it, and an identifier that moved would make all of
    // them point at nothing. The corrections are enumerated in rootCause, and the ledger test pins
    // them by text so the refuted wording cannot return through an edit nobody re-measures.
    id: "SD-8-genesis-exhibit-cannot-prove-well-formedness",
    title:
      "An exhibited preimage proves PREIMAGE KNOWLEDGE and HASH CONSISTENCY with the committed value, never that the bytes are a public key the admitted relation's OFF-CHAIN attestor will ever accept, so a principal can commit bytes of ANY length — ZERO included — at genesis, dormant rotation, dormant recovery and the arming edge, and hold a MANDATORY PQ commitment that cannot authorise under an honest attestor",
    property: null,
    rootsRequired:
      "0 — no principal is compromised, at any of the four sites. This is the DECLARED RESIDUAL of SD-7's remediation. At genesis the configuration is chosen by whoever deploys, and `deployVault` is permissionless, so a bad genesis lands at its own CREATE2 address and harms only that vault; at dormant rotation and at the arming edge the acting principal is the credential, which already holds the cut it acts at; at dormant recovery it is a guardian quorum at k, which already replaces the credential outright. Measured authority delta: ZERO at every site (Sd8KeyWellFormednessAdjudication.test.ts sections C and D).",
    contradicts:
      "Nothing published — it is recorded HERE, at the moment of SD-7's remediation, precisely so that closing SD-7 is not read as a stronger claim than it is. That remediation closes the structurally CONTRADICTORY genesis (no preimage of a non-zero commitment; the length leg it also added was itself removed by SD5-I); it does not and cannot close the semantically dead one. What this entry ITSELF previously asserted, and lane SD-8 refuted by measurement, is enumerated in rootCause so that the refuted wording is not republished as a claim about the current kernel.",
    rootCause:
      "FIVE PROPERTIES, KEPT APART BECAUSE COLLAPSING THEM IS HOW THE EARLIER WORDING WENT WRONG. (1) PREIMAGE KNOWLEDGE — whoever calls supplies the bytes. (2) HASH CONSISTENCY — keccak256(bytes) equals the committed value: the kernel's ONLY check on key bytes, at five sites (initialize, _authorise, _requireIncomingPossession on both its dormant and armed branches, and the setVerifier arming edge; AST census: zero .length reads, zero algorithmId calls, zero reads of the declared floor lengths). (3) OPAQUE-BYTE COMMITMENT — the admitted Generation-1 class, ImmutableAttestationPQCVerifier, reads publicKey ONCE, as keccak256(publicKey), and compares it with the hash inside an EIP-712 statement its immutable attestor signed; on chain the key is opaque bytes whose hash is attested, and NO verifier judges the bytes under this relation. (4) ATTESTOR VERIFICATION — well-formedness and signature validity are judged only by the attestor's off-chain verifier (src/verifier/ml-dsa-65.ts), at attestation time, never at commitment time; for ML-DSA-65, the class's ATTESTED-ML-DSA-65 relation, well-formedness is exactly `length == 1,952` (every 1,952-byte string decodes; every other length throws). (5) SECRET-KEY POSSESSION — established only by that same off-chain signature check, and only where a possession proof is demanded at all. FOUR CORRECTIONS TO THIS ENTRY'S EARLIER WORDING, each measured: (i) the judging party is the trusted OFF-CHAIN ATTESTOR — this entry formerly named the on-chain verifier, which under the admitted relation cannot judge the bytes: an honest attestor refuses every malformed construction (the spend dies VerifierDenied) and a blind attestor, a constructed control, admits every one of them, the empty key included; (ii) this entry formerly said the garbage had to be of the correct length — bytes of ANY length are admitted, ZERO included, and for the EMPTY string the exhibit is vacuous because an omitted witness and an exhibited empty key are the same calldata; (iii) this entry formerly named genesis alone — the identical on-chain gap is at FOUR sites, genesis, dormant rotation, dormant recovery and the arming edge (setVerifier requirePq false -> true exhibits a preimage and asks no verifier), while armed rotation and armed recovery refuse malformed material ONLY because the attestor's off-chain check sits inside the possession proof; (iv) this entry formerly quoted the ML-DSA-44 public-key length as the size of the noise keccak256 cannot distinguish from a key — no admitted class implements that parameter set; the Generation-1 class is ML-DSA-65, whose public key is 1,952 bytes. What stands from the earlier wording: keccak256 is scheme-agnostic by construction, and the kernel performs no structural validation of key bytes anywhere, deliberately, because doing so would bind it to one PQ scheme.",
    // STATE_INCOHERENCE, RETAINED AFTER RE-MEASUREMENT AND NOW STATED PRECISELY. The incoherence is
    // between a commitment the floor declares MANDATORY (requirePq) and the acceptance set of the
    // relation that commitment is supposed to name: the bytes are not a credential of the admitted
    // relation, and no on-chain layer can tell. That is the family SD-6, SD-7 and SD-11A/B were
    // classified in — admitted state that does not carry the property the published semantics
    // attribute to it. It is NOT LIVENESS_DENIAL: every entry in that family has a principal
    // depriving another beyond a declared bound, and this one has roots 0 and no bound; the
    // liveness outcome is the incoherence's manifestation and is recorded in
    // notAnEscalationBecause, exactly as SD-7 recorded it. A third value was not added: it would
    // widen the receipt vocabulary for a distinction the existing pair already carries.
    classification: "STATE_INCOHERENCE",
    notAnEscalationBecause:
      "It reduces no cut at any site and is unreachable by an attacker against someone else's vault: the whole genesis is bound into the CREATE2 salt, so a different configuration is a different address (`I-COUNTERFACTUAL-IDENTITY-BINDING`; every malformed construction was measured at an address distinct from the honest one and from each other), and the other three sites are exercised only by the principal that already holds them. Measured consequence under an honest attestor: a self-inflicted, cut-0 liveness outcome — execute reverts VerifierDenied; NO downgrade (the ECDSA factor alone also reverts VerifierDenied — the conjunct is mandatory and unsatisfiable, not bypassed); NO false proof of possession; the credential cannot repair its own vault (rotateCredential and setVerifier are HYBRID-authorised), and it is escapable at k — a guardian quorum recovers such a vault and installs material it actually holds, executed rather than argued. The floor's declared pqPublicKeyLength beside a preimage of another length is documentary only: E-PRIME de-authorised that field and nothing reads it. The residue over SD-7 is narrow: an ACCIDENTAL misconfiguration is refused at birth, and only a DELIBERATE one survives, which is indistinguishable from a deployer who commits a real key and then destroys it. Attestor compromise dominates everything this entry touches — a blind attestor forges every spend outright — so nothing here is an SD-8 gain.",
    minimalFixSketch:
      "No on-chain fix establishes what is missing, and that is the finding rather than an omission: lane SD-8 measured the candidates without implementing any, and STOPPED on its brief's stop conditions (the trusted-attestor model makes the property not independently establishable on chain; no security or liveness defect beyond the declared one). (a) A kernel-held allowlist of trusted verifiers: introduces a governance principal this kernel does not have; SD-11 already established class provenance WITHOUT a registry, and a registry is forbidden by AUTHORITY.md's 'KERNEL ADMIN — DOES NOT EXIST'. (b) Scheme-aware validation of key bytes IN THE KERNEL (a fixed 1,952 gate): proves a length alone, admits every 1,952-byte non-key exactly as dead, binds the scheme-agnostic kernel to one parameter set, and is the family SD5-D1 already rejected. (b') Validation SUPPLIED BY THE ADMITTED CLASS, an isWellFormedKey view the kernel consults: the only on-chain control with no new principal, and NOT self-certifying after SD-11 because the class is root-fixed — but it still proves a length and nothing more, and it changes the byte-identical production class, re-opening SD-11's class assurance; an owner decision, not warranted on this evidence. (c) A proof of possession at genesis, at a dormant install or at the arming edge: the class is root-fixed but the ATTESTOR is chosen by the deployer (it is the root's CREATE2 salt), so the deployer's own attestor 'proves possession' of the empty key — self-certification, measured — and it adds attestor liveness to genesis. (d) An attestor-signed validity statement at commitment time: dominated by (c) and by the attestation every spend already carries. (e) Opaque bytes, the status quo: correct to keep. Every on-chain candidate collapses to a length check or to a delegation to the attestor.",
    reproducedBy:
      "prototype/vnext-kernel/test/Sd67CommitmentAdmission.test.ts (the genesis form: a 32-byte non-key on a 32-byte floor); prototype/vnext-kernel/test/Sd8KeyWellFormednessAdjudication.test.ts (eight constructions from 0 to 1,953 bytes against the real admitted class, an honest and a blind attestor on the same bytes, all four sites, and the candidate discriminators)",
  },
  {
...
export interface RemediatedDefect {
  ...
  /**
   * Set ONLY when the closure holds under a NAMED deployment condition. The receipt then publishes
   * DEFECT_CONDITIONALLY_REMEDIATED with this text beside it, never a bare DEFECT_REMEDIATED (lane
   * SD-11). A condition is not a residual: a residual names a SUSTAINED defect, while a condition is
   * a fact about how the system must be deployed for the verdict to be true.
   */
  condition?: string;
}
...
export const REMEDIATED_DEFECTS: readonly RemediatedDefect[] = [
  {
    ...
    // LANE SD-11: SD-11A is now CONDITIONALLY REMEDIATED, and this file's contract is that a residual
    // names a SUSTAINED defect, so the pointer moves to SD-4 — the item in the list above that this
    // removal NARROWED and did not close. SD-8 is unchanged and still not this entry's residual.
  ...
  },
  {
    ...
    title:
      "Generation 1 pinned no scheme: the kernel admitted any address with code and never established that the admitted verifier exposes only ONE accepting relation over a committed key, so a verifier carrying a forgeable second relation reduced the credential cut from 2 to 1 on the vaults that admitted it",
    sustainedAt: "be1789f42b4b4abb6aa071f3f15a22333059e60b",
    remediatedOn:
      "b058715b612be5ac70e44dfe5b8d4db1766ee571 (security/vnext-sd11-verifier-admission-provenance, lane SD-11 GREEN; its successor 40ba7059a5aa475cf12c675480ca472fb9f9831c is comment-only, executable prefix byte-identical)",
    invariant:
      "G-VERIFIER-ADMISSION-PROVENANCE — a verifier may become ACTIVE only if (1) it belongs to a Generation-1-approved implementation class, (2) its accepting-relation configuration is immutable after deployment, (3) that fact is mechanically attributable rather than documentary, and (4) every path that can change the active verifier applies the same check. For SD-11A the operative clause is (1). The kernel still does not establish relation multiplicity for arbitrary bytecode, and no longer needs to, because it refuses arbitrary bytecode: it establishes CLASS MEMBERSHIP by provenance — the root named in the vault's own clone args must have CREATED the candidate — and the Generation-1 root can create exactly one class, ImmutableAttestationPQCVerifier, whose single accepting relation is an attestor-signed EIP-712 statement over the digest, the committed key's hash, the verifier address and the chain.",
    sourceDelta:
      "Additive only: no existing statement removed, no new kernel state, no new kernel selector, and no owner or setter anywhere. KERNEL: error InadmissibleVerifier(); internal view _requireAdmissibleVerifier(address), one STATICCALL to IKernelVerifierAuthority(root).isAdmissibleVerifier(candidate) that fails closed when no root is bound; internal view _verifierAuthority(), bytes 8..28 of the clone's immutable args; one call at initialize, setVerifier and initiateRecovery, after the existing code-length check and before any gate, nonce or write. FACTORY: third constructor argument verifierAuthority (refused if zero or codeless), immutable, public getter, appended to the clone args after the 8-byte generation. NEW CONTRACTS: ImmutableAttestationVerifierFactoryPrototype, the Generation-1 root, whose deployVerifier(attestor) creates the class by CREATE2 with the attestor as salt and records isAdmissibleVerifier in the same function, with no other writer and no constructor parameter; byte-identical copies of contracts/IPQCVerifier.sol and contracts/verifiers/ImmutableAttestationPQCVerifier.sol inside the scanner-scoped tree; IKernelVerifierAuthority in IKernelPlanes.sol; UngatedVerifierAuthority in PrototypeMocks.sol, the pre-lane code-length rule as a named fixture for suites that test other properties. MEASURED with the pinned solc (0.8.24, cancun, optimizer 200): kernel runtime 17,695 -> 17,964 B (+269), storage 17 entries / 11 slots unchanged, 46 selectors and 15 events unchanged, errors 24 -> 25; factory runtime 2,445 -> 2,551 B, selectors 4 -> 5; root runtime 4,196 B; clone runtime 53 -> 73 B.",
    rejectedAlternatives:
      "AN EXTCODEHASH PIN — refuted twice by execution: lane SD-11 C4 changed the relation with bit-identical runtime, and this lane's M5-A deployed a verifier with the SAME codehash as an admitted one, outside the root, which a pin would admit. AN OWNER- OR GOVERNANCE-CURATED REGISTRY of approved addresses or codehashes — forbidden by the owner decision and by AUTHORITY.md's 'KERNEL ADMIN — DOES NOT EXIST'; it is the principal SD-8's minimalFixSketch already refused, and mutant M7-A2 (a root with one extra open writer) shows how little such a writer needs to be to reopen SD-11A. CLASS IDENTITY BY A RUNTIME-SKELETON HASH IN THE KERNEL (mask the immutables, hash the rest) — rejected: it pins one compilation including its metadata, admits instances no constructor produced, puts brittle constants in the kernel, and would force every suite onto one verifier class, discarding the EcdsaBackedVerifier evidence whose value is a real possession requirement. A CREATE2 RECOMPUTATION WITHOUT A RECORD — rejected: it proves an address the root COULD produce, not that the root PRODUCED the code there, so an account reaching that address by a colliding route would pass; the record is written only after the root's own CREATE2 succeeded. THE ROOT AS GENESIS CONFIGURATION — rejected: a per-vault deployer choice, so a generation could not promise what it admits. THE ROOT AS A KERNEL IMMUTABLE — rejected by I-PURE-CONSTRUCTOR (zero immutables, one runtime hash per generation). THE ROOT CREATED INSIDE THE FACTORY'S CODE rather than passed to its constructor — sound, and not chosen: the factory's runtime already embeds the bound root as an immutable, so its code identity names the root either way, while hard-coding it would evict every mock-verifier suite from the real factory. IMPORTING THE PRODUCTION CLASS instead of copying it — rejected: scanner-input-scope.ts follows only remapped imports from the contracts tree, so an out-of-tree import would escape the scanner currency licence; the copy is byte-identity-tested instead. A SECOND CHECK IN executeRecovery — rejected as redundant: an AST census proves initiateRecovery is the only writer of the proposal executeRecovery installs, and provenance cannot lapse in between. ADMITTING EcdsaBackedVerifier INTO GENERATION 1 — rejected as a policy invention: it is a test stand-in, not a supported implementation class. FORCING ZKMLDSAVerifier THROUGH THE GATE — not attempted: its semantics depend on an external SP1 verifier this repository does not pin.",
    invertedReproduction:
      "prototype/vnext-kernel/test/Sd11VerifierAdmissionProvenance.test.ts — M2: the lane SD-11 dual-relation verifier, with code, the verifier ABI and a working strong relation, is REFUSED (InadmissibleVerifier) at genesis, setVerifier and initiateRecovery, each refusal paired with a positive control on the same vault, and its weak relation is asserted STILL LIVE afterwards; M1 and M5 refuse arbitrary code, an identical-codehash copy, a self-asserting verifier and a counterfeit root's approvals; M6 carries the AST census (pqVerifier writers = initialize, setVerifier, executeRecovery; proposal writer = initiateRecovery; each check examines the written value before the write); M7-K1, M7-K2 and M7-K3 delete the check at each edge and are killed only on the observation that the hostile verifier becomes ACTIVE, and M7-K2 then moves value with the ECDSA root alone, the exact SD-11A cut reduction. prototype/vnext-kernel/test/Sd11VerifierAdmissionSemantics.test.ts section B is NOT inverted and runs unchanged: it binds UngatedVerifierAuthority, the pre-lane admission rule, so the hostile semantics and their historical reachability stay executable evidence rather than being neutralised.",
    residual: null,
    condition:
      "GENERATION BINDING — the closure holds on vaults whose clone args bind the Generation-1 root (ImmutableAttestationVerifierFactoryPrototype), i.e. every vault a VaultKernelFactoryPrototype constructed with that root creates, and it is checkable offline from genesisCommitments() bytes 8..28 and the root's code. The kernel IMPLEMENTATION does not pin the root, so the same bytes bound to another root admit what that root approves (UngatedVerifierAuthority reproduces this on purpose): that is D8's generation-publisher residual, H-32, not a principal with authority over any existing vault. CLASS ASSURANCE — 'one accepting relation' is established for ONE class by review of contracts/verifiers/ImmutableAttestationPQCVerifier.sol (a byte-identical copy, identity asserted in M4), not proven on chain for arbitrary bytecode; Generation 1 claims nothing about the attestor's honesty, which remains the class's declared trust root.",
  },
  {
    id: "SD-11B-admitted-verifier-semantic-immutability-is-not-established",
    title:
      "The kernel pinned no code identity for an admitted verifier and never re-validated it, so an already-admitted verifier COULD change its accepting relation in place — behind a stable address, with identical runtime bytecode and no kernel-visible event. MEASURED on the repository's own AttestationPQCVerifier, whose owner rotates the attestor that verify() reads",
    sustainedAt: "be1789f42b4b4abb6aa071f3f15a22333059e60b",
    remediatedOn:
      "b058715b612be5ac70e44dfe5b8d4db1766ee571 (security/vnext-sd11-verifier-admission-provenance, lane SD-11 GREEN; its successor 40ba7059a5aa475cf12c675480ca472fb9f9831c is comment-only, executable prefix byte-identical)",
    invariant:
      "G-VERIFIER-ADMISSION-PROVENANCE, as stated in SD-11A's entry. For SD-11B the operative clauses are (2) and (4): the only admissible class holds its accepting-relation configuration in CODE — attestor is immutable — and its runtime can never write storage, borrow code, call out with value, create or destroy itself, so changing the relation requires a new deployment at a new address; and every transition that can install a different address (initialize, setVerifier, initiateRecovery then executeRecovery) re-enters the admission check. The binding cannot move either: the root lives in the clone's code, and the root holds no power over any verifier it created. The kernel still re-validates nothing after admission, and does not need to: nothing it admits can change.",
    sourceDelta:
      "Shared with SD-11A-verifier-semantic-admission-is-unverifiable-by-the-kernel, whose sourceDelta is the complete delta; this entry adds no Solidity of its own.",
    rejectedAlternatives:
      "Shared with SD-11A, plus two specific to this entry. RE-VALIDATING THE VERIFIER AT AUTHORISATION (a codehash re-check in _authorise, the bindMigration/egress pattern) — rejected: the reachable mechanism leaves bytecode bit-identical (lane SD-11 C4 and G2: the mutable attestor lives in STORAGE), a delegating design keeps its codehash while its implementation moves (D1), a later-transaction SELFDESTRUCT cannot remove admitted code under cancun (E1), and it would add a read to every authorisation to detect nothing the admission rule lets in. PATCHING AttestationPQCVerifier (removing or timelocking updateAttestor) — out of scope and not a kernel property: production verifier behaviour is unchanged, and M3 asserts updateAttestor still works after Generation 1 refuses the contract.",
    invertedReproduction:
      "prototype/vnext-kernel/test/Sd11VerifierAdmissionProvenance.test.ts — M3: the REAL contracts/verifiers/AttestationPQCVerifier.sol is REFUSED at genesis, setVerifier and initiateRecovery, including when it names the vault's own attestor and accepts that attestor's statement at the moment of refusal, and its owner then still rotates the attestor (the mechanism is untouched; admission is what moved); M4: a root-created ImmutableAttestationPQCVerifier is ADMITTED at all three edges end to end, is executable-code-identical to the production class, exposes no writing entry point, contains no SSTORE, DELEGATECALL, CALLCODE, CALL, CREATE, CREATE2 or SELFDESTRUCT instruction (a linear sweep whose controls flag the mutable verifier, a proxy and a destructible contract), and keeps its identity and relation across every root and kernel operation that is not an admission; M5-D: no admission transition rewrites the clone's code, so the root every check reads cannot move. prototype/vnext-kernel/test/Sd11VerifierAdmissionSemantics.test.ts section C runs unchanged under UngatedVerifierAuthority and still reproduces the in-place rotation, which is what an ungated admission rule allows.",
    residual: null,
    condition:
