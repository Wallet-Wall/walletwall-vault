...
export const SUSTAINED_DEFECTS: readonly SustainedDefect[] = [
  ...
  },
  {
    ...
    reproducedBy:
      "prototype/vnext-kernel/test/Sd11VerifierAdmissionSemantics.test.ts section B. THE DEBT THIS FIELD RECORDED IS DISCHARGED, and the sentence it used to end with is kept here because it is what forced the work: 'that record is UNTRACKED lane scratch, so this entry names no tracked deterministic reproduction. Promoting one is owed by the lane that acts on SD-11A.' Section B re-derives the construction independently of that scratch and executes it with a REALLY DEPLOYED dual-relation verifier and no setCode: B1 one address accepts a strong witness and a forgeable weak one and refuses a wrong one (the vacuity guard), with the weak leg placed AT the declared 65-byte shape so no length gate could have separated them (M6); B2 it is ADMITTED by a real HYBRID setVerifier at cut 2; B3 the ECDSA root ALONE then moves value; B4 the kernel's whole observation is one bool, so it cannot tell which relation answered; B5 the same weak leg also carries rotateCredential, so the reduction is not spend-only; B6 the IDENTICAL forged witness is REFUSED by the honest single-relation verifier, which attributes the success to the admitted verifier rather than to the kernel. The original matrix (M1-M6, N0-N5) remains in prototype/vnext-kernel/SD5_A1R_ADVERSARIAL_CLOSURE.scratch.md, still untracked and still in another worktree. NOT CLAIMED, here or there: that any PRODUCTION candidate exposes two accepting relations. None examined does.",
  },
  {
    id: "SD-11B-admitted-verifier-semantic-immutability-is-not-established",
    title:
      "The kernel pins no code identity for an admitted verifier and never re-validates it, so an already-admitted verifier CAN change its accepting relation in place — behind a stable address, with identical runtime bytecode and no kernel-visible event. MEASURED on the repository's own AttestationPQCVerifier, whose owner rotates the attestor that verify() reads",
    property: null,
    rootsRequired:
      "1 — the ADMITTED VERIFIER'S OWN OWNER KEY, a principal the kernel's authority tables do not enumerate at all (AUTHORITY.md section 1 lists the PQ verifier as holding 'none'). That root moves the accepting relation; reaching ASSETS additionally needs the vault's ECDSA credential root, because the verifier is a conjunctive plane and cannot authorise by itself. SUPERSEDES this field's previous value, 'UNKNOWN, and that is the entry', which also named the wrong mechanism family: it said the answer depended on the verifier being 'upgradeable, proxied or metamorphic', and the mechanism actually found reachable is NONE OF THOSE THREE — it is ordinary owner-controlled STORAGE behind unchanged code. For the ZK path the answer remains genuinely UNKNOWN: ZKMLDSAVerifier holds SP1_VERIFIER immutable but FORWARDS to it, and docs/ZK_Prover_Runbook.md directs operators to point that address at an SP1 gateway this repository neither pins nor measures.",
    contradicts:
    ...
    rootCause:
      "Proven from source in lane SD5-I, and the proof is a CONTRAST INSIDE THE SAME CONTRACT. The verifier is checked for a nonzero code length at admission (initialize, setVerifier, initiateRecovery) and never again; no codehash is recorded or compared. Yet bindMigration DOES pin code identity — it reverts DestinationMismatch when destinationVault.codehash differs from the bound destinationVaultCodeHash — so the kernel possesses the mechanism and applies it elsewhere, but not to the verifier. IKernelPQVerifier.verify is reached by STATICCALL against a stored address, so a delegatecall proxy could change implementation with nothing the kernel observes. THE MECHANISM THAT WAS ACTUALLY MEASURED IS SIMPLER THAN THAT, AND THE DIFFERENCE MATTERS BECAUSE IT DEFEATS A DIFFERENT SET OF CONTROLS: no proxy is involved. AttestationPQCVerifier.verify RECOVERS an EIP-712 signer and compares it to the STORAGE variable `attestor`, which updateAttestor(address) rewrites under onlyOwner. The relation the kernel consults is therefore a function of storage the kernel never reads, behind runtime bytecode that never changes — so address identity and code identity BOTH remain constant across a complete change of the accepting relation. Where the authority physically lives is itself measured rather than inferred from the word `immutable` (section G2): the mutable variant's attestor is found in a storage slot and NOT in its runtime code, and the immutable variant's is in its runtime code and NOT in storage.",
    classification: "STATE_INCOHERENCE",
    notAnEscalationBecause:
      "NO DECLARED CUT MOVES, and the reason is narrower than it used to be. This field previously read 'No cut is claimed to move, because no reachability is claimed at all. This is an UNMEASURED CLASS' — the second half is now false and the first half survives on its own merits. The verifier is a CONJUNCTIVE plane: it can deny and it can fail to deny, never authorise alone, so moving its relation does not by itself reach assets. What the rotation does is SUBSTITUTE the authority set: the second factor moves from 'holder of the committed PQ key' to 'whoever the verifier's owner names', while the COUNT stays at 2 because the ECDSA credential is still required. Against the architecture's declared envelope this is the PLANE-SAFE outcome already accepted at docs/Vault_vNext_Architecture.md:397 (Byzantine verifier => bounded by the floor). WHAT IS NEW IS TIMING AND OBSERVABILITY: the verifier need not be Byzantine when it is admitted, the substitution needs no kernel transaction, and the kernel emits nothing.",
    minimalFixSketch:
      "REACHABILITY IS NOW ESTABLISHED, so this entry no longer asks for it. What the measurement changes is which controls are available. A CODEHASH PIN IS REFUTED AS A CLOSURE, by execution rather than by argument: the reachable case leaves runtime bytecode BIT-IDENTICAL (the attestor lives in storage, measured in section G2), so a pin sees nothing; it also fails against a delegatecall design, whose proxy codehash is stable while the implementation pointer moves (section D1, a CONSTRUCTED_CONTROL); and its nominal target is empty for an ALREADY-ADMITTED verifier, because under the pinned cancun EVM a later-transaction SELFDESTRUCT sweeps the balance and LEAVES THE CODE (EIP-6780, section E1). The earlier wording — that a pin 'would address metamorphic redeploy' — is therefore superseded in both directions. The only control that closes the reachable case is a restriction on WHICH VERIFIERS MAY BE ADMITTED: an admission precondition (c) that the admitted verifier exposes no post-admission authority over its accepting relation. That property is NOT a function of any on-chain identity the kernel can read, so it is an off-chain artifact-assurance obligation, and choosing whether Generation 1 takes it on is an OWNER decision, not a kernel mechanism. Of the verifiers implemented here, only ImmutableAttestationPQCVerifier satisfies it today. Scope is unchanged: this is still not licence for a broad proxy/metamorphic research campaign.",
    reproducedBy:
      "prototype/vnext-kernel/test/Sd11VerifierAdmissionSemantics.test.ts section C, against the REAL contracts/verifiers/AttestationPQCVerifier.sol and ImmutableAttestationPQCVerifier.sol compiled from their own sources and deployed by ordinary transactions — no setCode, asserted mechanically in section F. SUPERSEDES 'NONE — deliberately'. The five required facts: (A) the verifier is admitted at genesis; (B) the kernel-visible address is unchanged across the mutation; (C) a spend attested by a second attestor is refused BEFORE and accepted AFTER; (D) the only transaction in between is updateAttestor on the VERIFIER — the kernel emits zero logs and consumed no nonce on the refusal; (E) the kernel then moves value on the changed relation. C6 additionally shows the relation MOVED rather than widened: the original attestor, accepted before, is refused after. C7 is the positive control that keeps the class from being read as universal — ImmutableAttestationPQCVerifier exposes no mutator, and its fixed attestor still authorises.",
  },
  {
